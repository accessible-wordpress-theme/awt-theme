<?php
/**
 * Title: AWT — CTA section
 * Slug: awt/cta-section
 * Design system: carbon
 * Description: Centered headline + supporting text + primary action. Section break between content and conversion.
 * Categories: awt-section
 * Keywords: cta, call to action, conversion
 * Block Types: core/post-content
 * Inserter: yes
 */
?>
<!-- wp:awt/section {"paddingBlock":"10","maxWidth":"narrow"} -->
<!-- wp:heading {"level":2,"textAlign":"center"} -->
<h2 class="wp-block-heading has-text-align-center">Ready to ship accessible by default?</h2>
<!-- /wp:heading -->

<!-- wp:paragraph {"align":"center"} -->
<p class="has-text-align-center">Install the theme and the blocks plugin from WordPress.org. No license keys, no telemetry, no callbacks. The accessibility linter ships in the box.</p>
<!-- /wp:paragraph -->

<!-- wp:awt/button {"text":"Get started","kind":"primary","size":"lg","href":"#"} /-->
<!-- /wp:awt/section -->
